...
class HtmlTreeBuilder extends TreeBuilder {
    ...
    List<Node> parseFragment(String inputFragment, Element context, String baseUri, ParseErrorList errors) {
        ...
        if (context != null) {
            ...
            root = new Element(Tag.valueOf("html"), baseUri, 0);
    ...
    Element insert(Token.StartTag startTag) {
        ...
        if (startTag.isSelfClosing()) {
            ...
            tokeniser.emit(new Token.EndTag(el.tagName(), -1));  // ensure we get out of whatever state we are in. emitted for yielded processing
            return el;
        }
        
        Element el = new Element(Tag.valueOf(startTag.name()), baseUri, startTag.attributes, startTag.getPos());
        insert(el);
        return el;
    }

    Element insert(String startTagName) {
        Element el = new Element(Tag.valueOf(startTagName), baseUri, -1);
        insert(el);
        return el;
    }
    ...
    Element insertEmpty(Token.StartTag startTag) {
        ...
        Element el = new Element(tag, baseUri, startTag.attributes, startTag.getPos());
        insertNode(el);
        if (startTag.isSelfClosing()) {
            if (tag.isKnownTag()) {
    ...
    FormElement insertForm(Token.StartTag startTag, boolean onStack) {
        Tag tag = Tag.valueOf(startTag.name());
        FormElement el = new FormElement(tag, baseUri, startTag.attributes, startTag.getPos());
        setFormElement(el);
        insertNode(el);
        if (onStack)
            stack.add(el);
    ...
    void insert(Token.Comment commentToken) {
        Comment comment = new Comment(commentToken.getData(), baseUri, commentToken.getPos());
        insertNode(comment);
    }
    ...
    void insert(Token.Character characterToken) {
        ...
        if (StringUtil.in(currentElement().tagName(), TagsScriptStyle))
            node = new DataNode(characterToken.getData(), baseUri, characterToken.getPos());
        else
