...
enum HtmlTreeBuilderState {
    Initial {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
                ...
                DocumentType doctype = new DocumentType(d.getName(), d.getPublicIdentifier(), d.getSystemIdentifier(), tb.getBaseUri(), d.getPos());
    ...
    BeforeHead {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
                ...
                tb.process(new Token.StartTag("head", -1));
                return tb.process(t);
            } else if (t.isEndTag()) {
                tb.error(this);
                return false;
    ...
    InHead {
        ...
        private boolean anythingElse(Token t, TreeBuilder tb) {
            tb.process(new Token.EndTag("head", -1));
            return tb.process(t);
        }
    },
    InHeadNoscript {
        ...
        private boolean anythingElse(Token t, HtmlTreeBuilder tb) {
            tb.error(this);
            tb.process(new Token.EndTag("noscript", -1));
            return tb.process(t);
        }
    },
    AfterHead {
        ...
        private boolean anythingElse(Token t, HtmlTreeBuilder tb) {
            tb.process(new Token.StartTag("body", -1));
            tb.framesetOk(true);
            return tb.process(t);
        }
    },
            ...
            switch (t.type) {
                ...
                case StartTag:
                    ...
                    if (name.equals("html")) {
                        ...
                        if (tb.inButtonScope("p")) {
                            tb.process(new Token.EndTag("p", -1));
                        }
                        tb.insert(startTag);
                        ...
                        for (int i = stack.size() - 1; i > 0; i--) {
                            ...
                            if (el.nodeName().equals("li")) {
                                tb.process(new Token.EndTag("li", -1));
                                break;
                            }
                        ...
                        for (int i = stack.size() - 1; i > 0; i--) {
                            ...
                            if (StringUtil.in(el.nodeName(), Constants.DdDt)) {
                                tb.process(new Token.EndTag(el.nodeName(), -1));
                                break;
                            }
                            if (tb.isSpecial(el) && !StringUtil.in(el.nodeName(), Constants.InBodyStartLiBreakers))
                                break;
                        ...
                        if (tb.inButtonScope("button")) {
                            // close and reprocess
                            tb.error(this);
                            tb.process(new Token.EndTag("button", -1));
                            tb.process(startTag);
                        } else {
                            tb.reconstructFormattingElements();
                            tb.insert(startTag);
                        ...
                        if (tb.getActiveFormattingElement("a") != null) {
                            tb.error(this);
                            tb.process(new Token.EndTag("a", -1));
                        ...
                        if (tb.inScope("nobr")) {
                            tb.error(this);
                            tb.process(new Token.EndTag("nobr", -1));
                            tb.reconstructFormattingElements();
                        }
                        Element el = tb.insert(startTag);
                        tb.pushActiveFormattingElements(el);
                        ...
                        tb.process(new Token.StartTag("form", -1));
                        if (startTag.attributes.hasKey("action")) {
                            Element form = tb.getFormElement();
                            form.attr("action", startTag.attributes.get("action"));
                        }
                        tb.process(new Token.StartTag("hr", -1));
                        tb.process(new Token.StartTag("label", -1));
                        // hope you like english.
                        String prompt = startTag.attributes.hasKey("prompt") ?
                                startTag.attributes.get("prompt") :
                                "This is a searchable index. Enter search keywords: ";

                        tb.process(new Token.Character(prompt, -1));
                        ...
                        tb.process(new Token.StartTag("input", inputAttribs, -1));
                        tb.process(new Token.EndTag("label", -1));
                        tb.process(new Token.StartTag("hr", -1));
                        tb.process(new Token.EndTag("form", -1));
                        ...
                        if (tb.currentElement().nodeName().equals("option"))
                            tb.process(new Token.EndTag("option", -1));
                        tb.reconstructFormattingElements();
                        tb.insert(startTag);
                ...
                case EndTag:
                    ...
                    if (name.equals("body")) {
                        ...
                        boolean notIgnored = tb.process(new Token.EndTag("body", -1));
                        if (notIgnored)
                        ...
                        if (!tb.inButtonScope(name)) {
                            tb.error(this);
                            tb.process(new Token.StartTag(name, -1)); // if no p to close, creates an empty <p></p>
                            return tb.process(endTag);
                        } else {
                        ...
                        for (int i = 0; i < 8; i++) {
                            ...
                            for (int j = 0; j < 3; j++) {
                                ...
                                Element replacement = new Element(Tag.valueOf(node.nodeName()), tb.getBaseUri(), node.getOffset());
                            ...
                            Element adopter = new Element(formatEl.tag(), tb.getBaseUri(), formatEl.getOffset());
                            adopter.setOffset(formatEl.getOffset());
                        ...
                        tb.error(this);
                        tb.process(new Token.StartTag("br", tb.reader.getPos()));
                        return false;
                    } else {
                        return anyOtherEndTag(t, tb);
                    }
        ...
        boolean process(Token t, HtmlTreeBuilder tb) {
            ...
            if (t.isCharacter()) {
                ...
                if (name.equals("caption")) {
                    ...
                    tb.process(new Token.StartTag("colgroup", pos));
                    return tb.process(t);
                    ...
                    tb.process(new Token.StartTag("tbody", -1));
                    return tb.process(t);
                } else if (name.equals("table")) {
                    tb.error(this);
                    boolean processed = tb.process(new Token.EndTag("table", pos));
    ...
    InCaption {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isEndTag() && t.asEndTag().name().equals("caption")) {
                ...
                tb.error(this);
                boolean processed = tb.process(new Token.EndTag("caption", -1));
                if (processed)
                    return tb.process(t);
    ...
    InColumnGroup {
        ...
        private boolean anythingElse(Token t, TreeBuilder tb) {
            boolean processed = tb.process(new Token.EndTag("colgroup", -1));
    ...
    InTableBody {
            ...
            switch (t.type) {
                case StartTag:
                    ...
                    if (name.equals("tr")) {
                        ...
                        tb.error(this);
                        tb.process(new Token.StartTag("tr", -1));
                        return tb.process(startTag);
        ...
        private boolean exitTableBody(Token t, HtmlTreeBuilder tb) {
            ...
            tb.clearStackToTableBodyContext();
            tb.process(new Token.EndTag(tb.currentElement().nodeName(), -1)); // tbody, tfoot, thead
            return tb.process(t);
        }
    ...
    InRow {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isStartTag()) {
                ...
                if (name.equals("tr")) {
                    ...
                    tb.process(new Token.EndTag("tr", -1));
                    return tb.process(t);
        ...
        private boolean handleMissingTr(Token t, TreeBuilder tb) {
            boolean processed = tb.process(new Token.EndTag("tr", -1));
            if (processed)
                return tb.process(t);
            else
                return false;
    ...
    InCell {
        ...
        private void closeCell(HtmlTreeBuilder tb) {
            if (tb.inTableScope("td"))
                tb.process(new Token.EndTag("td", -1));
            else
                tb.process(new Token.EndTag("th", -1)); // only here if th or td in scope
        }
    },
    InSelect {
        boolean process(Token t, HtmlTreeBuilder tb) {
                ...
                case StartTag:
                    ...
                    if (name.equals("html"))
                        ...
                        if (tb.currentElement().nodeName().equals("option"))
                            ...
                            tb.process(new Token.EndTag("optgroup", -1));
                        tb.insert(start);
                    } else if (name.equals("select")) {
                        tb.error(this);
                        return tb.process(new Token.EndTag("select", -1));
                    } else if (StringUtil.in(name, "input", "keygen", "textarea")) {
                        tb.error(this);
                        if (!tb.inSelectScope("select"))
                            return false; // frag
                        tb.process(new Token.EndTag("select", -1));
