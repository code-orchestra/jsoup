...
class Tokeniser {
    ...
    Token read() {
        ...
        if (charBuffer.length() > 0) {
            ...
            return new Token.Character(str, reader.getPos());
        } else {
            isEmitPending = false;
            return emitPending;
        }
    ...
    Token.Tag createTagPending(boolean start, int pos) {
        tagPending = start ? new Token.StartTag(pos) : new Token.EndTag(pos);
        return tagPending;
    }
    ...
    void createCommentPending(int pos) {
        commentPending = new Token.Comment(pos);
    }
    ...
    void createDoctypePending(int pos) {
        doctypePending = new Token.Doctype(pos);
    }
